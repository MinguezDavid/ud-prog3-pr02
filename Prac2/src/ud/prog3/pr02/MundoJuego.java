...
public class MundoJuego {
	...
	* crea una estrella nueva en una posición aleatoria y la añade al mundo y al panel visual */
	public void creaEstrella()
	{
		...
		int aleatorioX = (int)Math.random()*VentanaJuego.WIDTH;
		int aleatorioY = (int)Math.random()*VentanaJuego.HEIGHT;
			
		estrella.setPosicion( aleatorioX , aleatorioY );
		panel.add( estrella.getGrafico() );  // Añade al panel visual
		estrella.getGrafico().repaint();  // Refresca el dibujado del coche
		
		listaEstrellas.add( estrella );
		
		ultimaEstrella = Calendar.getInstance().getTimeInMillis();
	}
